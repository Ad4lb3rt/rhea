...
export function BaseToBase(input: string, inputBase: number, outputBase: number) : string
{
    let decimal : number = parseInt(input, inputBase);
    return Number(decimal).toString(outputBase);
}

export function TextToDecimal(input: string) : string
{
    let result : string = "";
...
export function DecimalToText(input: number[]) : string
{
    let result : string = "";
